...
export default function UpcomingTryouts({ userId }: UpcomingTryoutsProps) {
        ...
        if (userId) {
          const tryoutsWithStatus = await Promise.all(
            tryoutsData.map(async (tryout: any) => {
              try {
                const regResponse = await fetch(`/api/tryouts/register?tryoutId=${tryout.id}&userId=${userId}`);
  ...
  const getStatusBadge = (isRegistered: boolean, registrationStatus: string | null, typeTryout: string) => {
    if (isRegistered) {
      if (typeTryout === "free") {
        return (
  ...
  const getButtonText = (isRegistered: boolean, registrationStatus: string | null, typeTryout: string) => {
    if (isRegistered) {
      if (typeTryout === "free") {
        return "🚀 MULAI TRYOUT";
      } else if (registrationStatus === "approved") {
  ...
  const getButtonClass = (isRegistered: boolean, registrationStatus: string | null, typeTryout: string) => {
    if (isRegistered) {
      ...
      <div className="mt-6 sm:mt-8">
        <div className="bg-white border-3 sm:border-4 border-slate-800 p-4 sm:p-6 shadow-brutal">
          <div className="flex items-center justify-center py-8">
            ...
            <span className="ml-3 font-black text-slate-900">Memuat tryout tersedia...</span>
          </div>
        </div>
      </div>
    );
        ...
        <div className="bg-white border-3 sm:border-4 border-slate-800 p-4 sm:p-6 shadow-brutal">
          <div className="text-center py-8">
            ...
            <button
              ...
              className="mt-4 bg-purple-500 text-white px-4 py-2 font-black text-sm border-2 border-slate-800 hover:bg-purple-600 transition-colors"
            >
              Coba Lagi
            </button>
          </div>
      ...
      <div className="mt-6 sm:mt-8">
        <div className="bg-white border-3 sm:border-4 border-slate-800 p-4 sm:p-6 shadow-brutal">
          <div className="text-center py-8">
            ...
            <Link
              href="/tryouts"
              className="bg-purple-500 text-white px-4 py-2 font-black text-sm border-2 border-slate-800 hover:bg-purple-600 transition-colors inline-block"
            >
              Lihat Semua Tryout
            </Link>
          </div>
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3">
                ...
                <div className="text-left sm:text-right mb-2 sm:mb-0">
                  {getStatusBadge(tryout.isRegistered || false, tryout.registrationStatus || null, tryout.type_tryout)}
              ...
              <button
                ...
                disabled={tryout.isRegistered && tryout.type_tryout !== "free" && tryout.registrationStatus !== "approved"}
              >
                {getButtonText(
                  tryout.isRegistered || false,
                  tryout.registrationStatus || null,
